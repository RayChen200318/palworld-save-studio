    ...
    <section class="inventory-layout">
        ...
        <div v-else-if="activeContainer?.Slots.length" class="slot-grid" :class="`container-${items.selectedContainer}`">
          ...
          >
            ...
            <template v-if="slot.Item">
              ...
              <small>{{ slot.Item.StaticId }}</small>
      ...
      <aside class="panel detail-panel">
        <template v-if="selectedItem && selected">
          ...
          <div v-if="selectedItem.StateFlags.length" class="state-list"><span v-for="flag in selectedItem.StateFlags" :key="flag">{{ flag }}</span></div>
          ...
          <div v-if="items.selectedContainer === 'drop'" class="detail-actions"><button class="button primary" type="button" @click="retrieveDropItem"><AppIcon name="archive" :size="16" />{{ copy.items.retrieve }}</button></div>
          <div v-else class="move-block">
          ...
          <button class="button danger delete-button" type="button" @click="deleteOpen = true"><AppIcon name="trash" :size="16" />{{ copy.items.delete }}</button>
        </template>
...
<style scoped>
.items-toolbar{min-height:84px;padding:14px 16px;display:flex;align-items:center;gap:18px}.toolbar-field{width:260px}.toolbar-field .field-label{margin-bottom:5px}.toolbar-copy{min-width:0;flex:1}.toolbar-copy strong,.toolbar-copy span{display:block}.toolbar-copy strong{color:var(--text-strong);font-size:12px}.toolbar-copy span{margin-top:5px;color:var(--text-muted);font-size:10px}.container-tabs{margin:14px 0;display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:8px}.container-tabs button{height:56px;padding:0 12px;display:flex;align-items:center;gap:8px;border:1px solid var(--border-subtle);border-radius:11px;background:var(--surface-soft);color:var(--text-muted);cursor:pointer}.container-tabs button:hover{border-color:var(--border-bright);color:var(--text)}.container-tabs button.active{border-color:rgba(53,218,232,.38);background:rgba(27,183,203,.12);color:var(--cyan-300)}.container-tabs span{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:10px;font-weight:700}.container-tabs small{margin-left:auto;color:var(--text-faint);font-size:8px}.error-banner{margin:0 0 14px;padding:10px 12px;display:flex;align-items:center;gap:8px;border:1px solid rgba(240,120,120,.25);border-radius:10px;background:rgba(217,78,92,.08);color:var(--red-300);font-size:10px}.inventory-layout{display:grid;grid-template-columns:minmax(0,1fr) 340px;gap:14px;align-items:start}.inventory-panel{min-width:0}.slot-grid{padding:14px;display:grid;grid-template-columns:repeat(7,minmax(88px,1fr));gap:8px}.slot-grid.container-essential{grid-template-columns:repeat(8,minmax(80px,1fr));max-height:610px;overflow:auto}.slot-grid.container-food,.slot-grid.container-weapon{grid-template-columns:repeat(6,minmax(100px,1fr))}.slot-grid.container-armor,.slot-grid.container-drop{grid-template-columns:repeat(4,minmax(110px,1fr))}.item-slot{position:relative;min-width:0;height:112px;padding:18px 7px 7px;display:flex;flex-direction:column;align-items:center;border:1px solid var(--border-subtle);border-radius:11px;background:rgba(4,14,24,.52);color:var(--text-muted);cursor:pointer;overflow:hidden}.item-slot:hover{border-color:rgba(71,218,231,.32);background:var(--surface-hover)}.item-slot.selected{border-color:var(--cyan-400);box-shadow:0 0 0 2px rgba(35,199,216,.09),inset 0 0 22px rgba(35,199,216,.055)}.item-slot.locked{opacity:.36;cursor:not-allowed}.item-slot.anomalous{border-color:rgba(240,120,120,.36)}.slot-number{position:absolute;top:6px;left:7px;color:var(--text-faint);font-size:7px;font-weight:700;text-transform:uppercase}.item-slot img,.unknown-icon{width:43px;height:43px;display:grid;place-items:center;object-fit:contain}.unknown-icon{color:var(--gold-300)}.item-slot strong,.item-slot small{max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.item-slot strong{margin-top:4px;color:var(--text);font-size:8px}.item-slot small{margin-top:2px;color:var(--text-faint);font-size:6px}.item-slot b{position:absolute;right:6px;bottom:6px;padding:2px 4px;border-radius:5px;background:rgba(3,11,19,.82);color:var(--text-strong);font-size:8px}.item-slot i{position:absolute;right:6px;top:5px;width:15px;height:15px;display:grid;place-items:center;border-radius:50%;background:var(--red-400);color:white;font-size:8px;font-style:normal}.empty-mark{margin-top:13px;width:39px;height:39px;display:grid;place-items:center;border:1px dashed var(--border-subtle);border-radius:10px;color:var(--text-faint)}.inventory-empty{min-height:240px;display:grid;place-items:center;color:var(--text-muted);font-size:11px}.detail-panel{position:sticky;top:98px;min-height:430px;padding:18px}.item-detail-head{display:flex;align-items:center;gap:13px}.item-detail-head>img,.detail-unknown{width:62px;height:62px;display:grid;place-items:center;object-fit:contain;border:1px solid var(--border-subtle);border-radius:13px;background:rgba(255,255,255,.025)}.detail-unknown{color:var(--gold-300)}.item-detail-head>div{min-width:0}.item-detail-head span{color:var(--cyan-400);font-size:8px;font-weight:750;text-transform:uppercase}.item-detail-head h2{margin:5px 0 4px;color:var(--text-strong);font-size:15px}.item-detail-head code{display:block;max-width:220px;overflow:hidden;color:var(--text-faint);font-size:8px;text-overflow:ellipsis}.tag-row{margin:14px 0;display:flex;flex-wrap:wrap;gap:7px}.tag-row :deep(.status-pill){min-height:22px;font-size:8px}.state-list{margin-bottom:14px;display:flex;flex-wrap:wrap;gap:5px}.state-list span{padding:4px 6px;border-radius:6px;background:rgba(217,78,92,.09);color:var(--red-300);font-size:7px}.detail-fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.detail-fields label:only-child{grid-column:1/-1}.egg-card,.passive-card{margin-top:13px;padding:10px;border:1px solid var(--border-subtle);border-radius:10px;background:var(--surface-soft)}.egg-card{display:flex;align-items:center;gap:10px}.egg-card img{width:42px;height:42px;object-fit:contain}.egg-card span,.passive-card>span{display:block;color:var(--text-faint);font-size:8px;text-transform:uppercase}.egg-card strong{display:block;margin-top:4px;color:var(--text);font-size:10px}.passive-card code{display:block;margin-top:6px;color:var(--gold-200);font-size:8px}.move-block{margin-top:16px;padding-top:14px;border-top:1px solid var(--border-subtle)}.move-block>div{display:grid;grid-template-columns:1fr 1fr;gap:7px}.move-block .button{grid-column:1/-1;height:34px}.detail-actions{margin-top:16px}.detail-actions .button{width:100%}.delete-button{width:100%;margin-top:9px}.empty-detail{min-height:390px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center}.empty-detail>span{width:58px;height:58px;display:grid;place-items:center;border:1px solid var(--border-subtle);border-radius:15px;background:var(--surface-soft);color:var(--cyan-300)}.empty-detail h2{margin:15px 0 7px;color:var(--text-strong);font-size:14px}.empty-detail p{max-width:240px;margin:0 0 16px;color:var(--text-muted);font-size:10px;line-height:1.6}.catalog-filters{display:grid;grid-template-columns:minmax(0,1fr) 180px 160px;gap:8px}.catalog-search{height:40px;padding:0 11px;display:flex;align-items:center;gap:8px;border:1px solid var(--border-subtle);border-radius:9px;background:rgba(5,16,27,.62);color:var(--text-faint)}.catalog-search input{width:100%;border:0;outline:0;background:transparent;color:var(--text);font-size:11px}.add-layout{margin-top:12px;display:grid;grid-template-columns:minmax(0,1fr) 230px;gap:12px}.catalog-list{height:440px;padding-right:6px;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));align-content:start;gap:7px;overflow:auto}.catalog-list>button{min-width:0;height:68px;padding:8px;display:flex;align-items:center;gap:8px;border:1px solid var(--border-subtle);border-radius:10px;background:var(--surface-soft);color:var(--text);text-align:left;cursor:pointer}.catalog-list>button:hover,.catalog-list>button.selected{border-color:var(--cyan-400);background:rgba(35,199,216,.08)}.catalog-list img{width:43px;height:43px;flex:0 0 43px;object-fit:contain}.catalog-list button>span{min-width:0;flex:1}.catalog-list strong,.catalog-list small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.catalog-list strong{font-size:9px}.catalog-list small{margin-top:4px;color:var(--text-faint);font-size:7px}.catalog-list :deep(.status-pill){min-height:20px;padding:2px 5px;font-size:6px}.catalog-empty{grid-column:1/-1;padding:80px 0;color:var(--text-muted);text-align:center}.add-options{padding:15px;border:1px solid var(--border-subtle);border-radius:11px;background:rgba(4,14,24,.45)}.add-options>img{width:78px;height:78px;display:block;margin:0 auto;object-fit:contain}.add-options h3{margin:11px 0 4px;color:var(--text-strong);font-size:13px;text-align:center}.add-options>code{display:block;overflow:hidden;color:var(--text-faint);font-size:7px;text-align:center;text-overflow:ellipsis}.add-options label{display:block;margin-top:16px}.add-options p{margin:18px 0 0;color:var(--text-muted);font-size:9px;line-height:1.6}.delete-copy{margin:0;color:var(--text-muted);font-size:11px;line-height:1.7}
@media(max-width:1366px){.inventory-layout{grid-template-columns:minmax(0,1fr) 310px}.slot-grid{grid-template-columns:repeat(6,minmax(78px,1fr))}.slot-grid.container-essential{grid-template-columns:repeat(7,minmax(72px,1fr))}.item-slot{height:104px}.container-tabs button{padding:0 8px}.container-tabs span{font-size:9px}}
</style>
