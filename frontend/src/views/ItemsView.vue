...
function stateLabel(flag: string) {
  return flag === 'invalid-quantity' ? copy.value.items.invalidQuantity : flag
}
    ...
    <section class="inventory-layout">
      ...
      <aside class="panel detail-panel">
        <template v-if="selectedItem && selected">
          ...
          <div v-if="invalidQuantity" class="repair-card"><div><strong>{{ copy.items.invalidQuantity }}</strong><span>{{ copy.items.invalidQuantityHint }}</span></div><button class="button secondary" type="button" :disabled="draft.pending" @click="repairQuantity">{{ copy.items.repairQuantity }}</button></div>
