...
- `0.4.2` 版本在存档浏览器中增加 Windows 可用盘符切换，并支持直接输入
  盘符绝对路径或 UNC 网络共享路径。
- `0.4.3-beta.1` 预发布版新增明确的专用服务器模式、强制完整核心存档备份、
  源文件变更检测、结构校验、仅写入实际变化文件，以及更新后的物品别名、
  变体和图标。
...
`0.4.2` 仍是最新稳定版。`0.4.3-beta.1` 包含专用服务器保护流程重构，
但尚未通过所要求的真实 PalServer 验收，因此不宣称已经验证专用服务器
存档编辑。物品管理不包括基地仓库、世界容器、
