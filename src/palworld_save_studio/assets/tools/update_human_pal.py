...
def pal_t(internal_name):
    return {
        ...
        "Elements": [],
        "Attacks": {"EPalWazaID::Human_Punch": 1, "EPalWazaID::Weapon_Use": 1},
        "Human": True,
        "I18n": {
            "en": "",
            "zh-CN": "",
...
def extract_pals():
    ...
    response = requests.get(f"{urls["en"]}Humans")
    while response.status_code != 200:
        print(f"Failed to fetch {urls["en"]}Humans")
        time.sleep(5)
        response = requests.get(f"{urls["en"]}Humans")
    ...
    for card in cards:
        ...
        try:
            icon_url = card.find(
                ...
                {
                    "src": re.compile(
                        r"https://cdn\.paldb\.cc/image/Pal/Texture/PalIcon/NPC/.+\.webp"
                    )
                },
            )["src"]
            if icon_url:
            ...
            print(f"Failed to extract icon for {internal_name}: {e}")
            icon_url = None
    ...
    for lang in urls:
        ...
        if lang == "en":
            ...
            if basic_info_root:
                ...
                suitability_divs = basic_info_root.find_all(
                    "div",
            ...
            # find variants
            tribes_row = []
            tribes = detail_soup.find("h5", class_="card-title text-info", string="Tribes")
            if tribes:
                table = tribes.find_next("table")
                if table:
                    tribes_row = table.find_all("tr")
...
while len(pal_internal_names) > 0:
    ...
    try:
        pal_variants = extract_pal_details(internal_name, pal["I18n"]["en"], pal) or []
    except:
        print(f"Failed to extract details for {internal_name}")
        continue
    for variant_internal_name in pal_variants or []:
...
with open("tmp_human_pal_data.json", "w", encoding="utf-8") as file:
    file.write(pal_json)
