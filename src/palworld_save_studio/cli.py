...
from palworld_save_studio.utils import *
from palworld_save_studio.config import *

class InteractThread(threading.Thread):
    _instance = None
    banner_message = ColorConsoleFormatter.get_colored_msg("\nPalworld Save Studio interactive mode.\nType pal_help() for help or quit() to exit.")
    ...
    def run(self):
        LOGGER.info("Palworld Save Studio interactive mode started.")
        import code
        try:
...
def main():
    LOGGER.info("Palworld Save Studio CLI started.")
