...
from palworld_save_studio.utils import LOGGER, clamp, DataProvider
from palworld_save_studio.core.pal_objects import (
    PalObjects,
    PalGender,
    PalSuitability,
    get_nested_attr,
...
from palworld_save_studio.utils.util import type_guard
...
class PalEntity:
    ...
    def IsAwakening(self, value: bool) -> None:
        if value:
            if "bIsAwakening" not in self._pal_param:
                self._pal_param["bIsAwakening"] = PalObjects.BoolProperty(True)
            else:
                PalObjects.set_BaseType(self._pal_param["bIsAwakening"], True)
        else:
